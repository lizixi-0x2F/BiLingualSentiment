...
# 使用R2作为最佳模型选择指标的训练脚本
# 基于现有run_mixed_train.sh脚本修改

# 默认参数
DEVICE="cuda"
BATCH_SIZE=128
EPOCHS=20
MONITOR="r2"  # 明确使用R2作为选择指标
PATIENCE=5
LR=0.001
LAMBDA=0.7
...
OUTPUT_DIR="outputs/r2_best_model_${TIMESTAMP}"

# 创建输出目录
mkdir -p "$OUTPUT_DIR"

# 输出配置信息
echo "启动情感分析模型训练 (使用R2优化策略)..."
echo "========================================"
echo "设备: $DEVICE"
...
# 在screen会话中运行训练
echo "在screen会话中启动训练..."
SCREEN_NAME="r2_model_train_${TIMESTAMP}"
